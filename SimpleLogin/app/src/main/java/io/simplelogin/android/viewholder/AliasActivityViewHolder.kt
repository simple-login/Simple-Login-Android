...
class AliasActivityViewHolder(private val binding: RecyclerItemAliasActivityBinding) : RecyclerView.ViewHolder(binding.root) {
    companion object {
    ...
    fun bind(activity: AliasActivity) {
        ...
        when (activity.action) {
            Action.FORWARD -> {
                ...
                binding.iconImageView.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.ic_send_24dp))
                binding.iconImageView.setTint(R.color.colorPrimary)
            }

            Action.REPLY -> {
                binding.emailTextView.text = activity.to
                binding.iconImageView.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.ic_reply_24dp))
                binding.iconImageView.setTint(R.color.colorPrimary)
            }

            else -> {
                binding.emailTextView.text = activity.from
                binding.iconImageView.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.ic_block_24dp))
